   ...
   implements Runnable
 {
   ...
   private long copyStream(InputStream inputStream, OutputStream outputStream, MessageDigest md) throws IOException
   {
     ...
     try {
       ...
       //MetricsSaver.addValueWithError("S3DistCpCopyStreamDelay", stopWatch.elapsedTime(), e);
   ...
   public ProcessedFile downloadAndMergeInputFiles() throws Exception {
     ...
     while ((!finished) && (numRetriesRemaining > 0)) {
       ...
       try {
         ...
         if (numRetriesRemaining <= 0) throw e; 
       }
       finally {
   ...
   private void copyToFinalDestination(Path curTempPath, Path finalPath, ProcessedFile processedFile, FileSystem inFs, FileSystem outFs) throws Exception
   {
     ...
     if (Utils.isS3Scheme(outFs.getUri().getScheme())) {
       ...
       if (this.reducer.shouldUseMutlipartUpload()) {
         ...
         outStream = new MultipartUploadOutputStream(s3, Utils.createDefaultExecutorService(), this.reducer.getProgressable(), bucket, key, meta, chunkSize, getTempDirs(this.reducer.getConf()));
       }
       else {
