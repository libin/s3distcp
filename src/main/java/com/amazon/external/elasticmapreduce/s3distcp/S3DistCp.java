   ...
   implements Tool
 {
   ...
   private static String getHostName() {
     try {
       ...
       return addr.getHostName(); } catch (UnknownHostException ex) {
     }
     return "unknown";
   }
   ...
   private static boolean isGovCloud()
   {
     ...
     try {
       ...
       LOG.info("GET http://169.254.169.254/latest/meta-data/placement/availability-zone exception ", e);
     } finally {
   ...
   public int run(String[] args)
   {
     ...
     if (options.isHelpDefined()) return 0;
     return run(options);
   }
   ...
   public int run(S3DistCpOptions options) {
     ...
     try
     {
       ...
       fileInfoListing = new FileInfoListing(job, srcPath, inputPath, destPath, options.getStartingIndex().longValue(), manifestFile, previousManifest);
     }
