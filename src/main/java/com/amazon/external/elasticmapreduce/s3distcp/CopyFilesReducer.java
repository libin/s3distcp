     ...
     throws IOException
   {
     ...
     synchronized (this) {
       ...
       if (this.uncommitedFiles.size() > 0) {
         String message = String.format("Reducer task failed to copy %d files: %s etc", new Object[] { Integer.valueOf(this.uncommitedFiles.size()), ((FileInfo)this.uncommitedFiles.iterator().next()).inputFileName });
